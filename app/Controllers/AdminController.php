...
class AdminController extends Action
{
    public function verifica(){
        ...
        // 90bbc8345f198e167183074e4
        __set('senha', $_POST['senha']);
        if($usuario->verificaUsuario()){
            // header('location: /admin');
            // não é um jeito muito bom, melhor criar uma hash e usar a opção de cima
            $this->render('admin', 'layoutAdm');
        }else{
            header('location: /adminLogin?mensagem=erro ');
        }
    }
        ...
        if ($mensagem->validaMensagem()) {
            ...
            if (filter_var($this->email, FILTER_VALIDATE_EMAIL)) {
                try {
                    ...
                    $mail->Password = 'etqw jwlv bles btqu';                               //SMTP password
